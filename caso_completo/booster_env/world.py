...
class World(EzPickle):
    ...
    def _fireMainEngine(self, power: float, d_alpha: float):
        ...
        # !! self.state.nozzle_angle [rad]
        self.state.nozzle_angle += np.deg2rad(MAX_GIMBAL_VELOCITY) * d_alpha * (dt * K_time)
        self.state.nozzle_angle = np.clip(self.state.nozzle_angle, -np.deg2rad(MAX_GIMBAL_ANGLE), np.deg2rad(MAX_GIMBAL_ANGLE))
        ...
        thrust_x = thrust * (-sin + thrust_dispersion)
        thrust_y = thrust * (cos - thrust_dispersion)
    ...
    def _applyWind(self):
        ...
        wind_mag = (
                np.tanh(
                    np.sin(0.02 * self._windStep)
                    + (np.sin(np.pi * 0.01 * self._windStep))
                )
                * self._windPower
        )
        self._windStep += 1
    ...
    def _apply_fins_damping(self, damping_coefficient=1000):

        # Get the body's angular velocity
        angular_velocity = self.fuselage.angularVelocity
        # Calculate the damping torque
        damping_torque = -damping_coefficient * angular_velocity
        # Apply the damping torque to the body
        self.fuselage.ApplyTorque(damping_torque, wake=True)
