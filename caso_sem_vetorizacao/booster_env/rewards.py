...
class Reward:
    ...
    def _v8(self, **kwargs):
        ...
        """
            1. Recompensa baseado em vx e vy separadamente
            2. R_vx e R_alpha maior nos primeiros timesteps
            4. Punição por mudar direcao horizontal do movimento
        """
        assert self.time_penalization > 0
        ...
        K_min, K_max = 1, 1.5
        mean_steps = 100
        min_t, max_t = mean_steps/3, 2*mean_steps/3

        Kt_vx = 1#np.clip(((K_min-K_max)/min_t)*step+K_max, K_min, K_max)
        Kt_vy = np.clip(((K_max-K_min)/(max_t-min_t))*(step-min_t), K_min, K_max)

        vx = (abs(V[0]) / abs(self.V0[0])) * Kt_vx
        vy = abs(V[1]) / abs(self.V0[1]) * Kt_vy
        s = np.linalg.norm(X) / np.linalg.norm(self.X0)
        alpha = alpha * Kt_vx
        ...
        # If the x velocity zone changes from the last step, penalize it
        Vx_threshold = 1
        prev_vx_dir = 0 if abs(kwargs["previous_state"][X_DOT]) <= Vx_threshold else 1 * np.sign(
            kwargs["previous_state"][X_DOT])
        vx_dir = 0 if abs(V[0]) <= Vx_threshold else 1 * np.sign(V[0])
        if prev_vx_dir != vx_dir:
            reward -= 0.05  # Must be big enough to compensate the position shaping term
