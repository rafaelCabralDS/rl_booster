...
class BoosterTracker(Callback):

    def __init__(self, env_config, train_config):
        super().__init__(env_config, train_config)
        self.actor = None
        self.critic = None
        self.on_step = super()._on_step

    def agent(self):
        return self.actor

    def save_model(self, path):
        if self.actor is None or self.critic is None: return
    ...
    def _read_log_file(self):
        ...
        if not os.path.exists(path):
            raise Exception(f"Log file at {path} was not found")
        ...
        with open(path, "r") as log:
            ...
            for line in log.readlines():
                ...
                if "Iteration" in line and not len(row_buffer) == 0:  # new iter
                    data.append(row_buffer)
                    row_buffer = {}
                mapper = {
                    "Average Episodic Length": "lengths",
                    "Episodic Return": "returns",
                    "Average Loss": "loss",
                    "Timesteps So Far": "timesteps",
                }
                for key, value in mapper.items():
                    if line.startswith(key):
                        row_buffer[value] = float(line.split(f"{key}:")[1])

            # Do not remove
            if not len(row_buffer) == 0:  # new iter
                data.append(row_buffer)
        self._log_df = pd.DataFrame(data)
...
class Sb3BoosterTracker(Callback, BaseCallback):
    ...
    def agent(self): return self.model.policy
...
def success_metric(episode):
    best_final_distance = np.linalg.norm((
        episode.state_history[-2]["x"],
        episode.state_history[-2]["y"])) - 30  # 30 +- distancia cg chao
    best_final_vx = episode.state_history[-2]["Vx"]
    best_final_vy = episode.state_history[-2]["Vy"]
    best_final_alpha = episode.state_history[-2]["alpha"]

    return abs(best_final_vy) + abs(best_final_vx) + abs(best_final_distance) + abs(best_final_alpha)
...
class SuccessRateTrialCallback(BaseCallback):
    ...
    def eval_and_report(self) -> bool:
        self.success_rate, self.mean_rew, std_rew, best_eval_ep = eval_sb3_agent(
            self.model.policy,
            None,
            env_=self.eval_env,
            render=False,
        ...
        self.trial.report(success_metric(best_eval_ep), self.eval_idx)
        self.eval_idx += 1
    ...
    def _on_step(self) -> bool:
        ...
        # Eval
        if done and self.eval_freq > 1 and self.iteration % self.eval_freq == 0:
